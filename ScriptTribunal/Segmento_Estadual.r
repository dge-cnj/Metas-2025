...
# 17/09/2025: Validação da versão paralelizada.
...
Estadual.Meta02 <- function(dge) {
    ...
    t_1 <- "2022-01-01" # 1° grau
    t_2 <- "2023-01-01" # 2° grau
    t_jt <- "2023-01-01" # Juizados e turmas
    t_ant <- "2011-01-01"
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum2_a = if_else(mpu_meta == TRUE & sigla_grau == "G1" & dt_primeira_medida_protetiva >= t_1 & dt_primeira_medida_protetiva < t_f, 1, 0),
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum2_b = if_else(mpu_meta == TRUE & sigla_grau == "G2" & dt_primeira_medida_protetiva >= t_2 & dt_primeira_medida_protetiva < t_f, 1, 0),
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum2_c = if_else(mpu_meta == TRUE & (sigla_grau == "JE"|sigla_grau == "TR") & dt_primeira_medida_protetiva >= t_jt & dt_primeira_medida_protetiva < t_f, 1, 0),
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum2_ant = if_else(mpu_meta == TRUE & !func.procura.array(lista=c(39,49,108),base=dge,variavel="id_ultima_classe") & dt_primeira_medida_protetiva >= t_0 & dt_primeira_medida_protetiva < t_f, 1, 0),
...
Estadual.Meta03 <- function(dge, pre_processual) {
  ...
  t_f <- "2026-01-01"
  t_1 <- "2025-01-01" # Um ano antes de t_f (início do ano)
  t_2 <- "2024-01-01" # Dois anos antes de t_f (início do ano anterior)
...
Estadual.Meta04 <- function(dge) {
    ...
    # 2° parte (cujos limites estão no mês de outubro) # Não foi validado
...
Estadual.Meta08 <- function(dge) {
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum8_a = if_else(mpu_meta == TRUE & flag_violencia_domestica == TRUE & dt_primeira_medida_protetiva >= t_0 & dt_primeira_medida_protetiva < t_f, 1, 0),
    ...
    dge <- dge %>% 
        mutate(
            ...
            mpum8_b = if_else(mpu_meta == TRUE & flag_violencia_domestica == TRUE & dt_primeira_medida_protetiva >= t_0 & dt_primeira_medida_protetiva < t_f, 1, 0),
